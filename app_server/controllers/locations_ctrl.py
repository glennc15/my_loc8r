...
def add_review(request, location_id):
	...
	for required_key in ['name', 'rating', 'review']:
		if required_key not in request.json:
			return show_error(status_code=400)
	...
	if post_r.status_code == 201:
		...
		return show_error(status_code=post_r.status_code)



	# hard coded location record in case it's needed for testing:
